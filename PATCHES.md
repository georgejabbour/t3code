...
Vite 0.3.3 passes the configured file-watcher options to bundled compilation,
so the fork's Vite 0.3.0 dependency patch is no longer required. The native watcher
does not report source changes in the restricted test environment. The Tailwind test
uses polling, changes a real source file, and waits for the compiled update over the
